...
from tests.plugins.employee_api import delete_employee_using_api


def test_add_employee_without_login_details(page: Page, logged_in_page, request):
    employee = create_employee(logged_in_page)
    request.addfinalizer(lambda: delete_employee_using_api(logged_in_page, employee.employee_id))
